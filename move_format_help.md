...
database directory: Database directory where you want to move uploaded file. Cannot be empty! 
...
source filename: Source filename. Star wildcard can be used. If empty, all uploaded files will be moved to Database directory defined before. 

regexp_pattern: If empty, destination filename is equal to source name. 
If not empty, source filename is equal to regexp_replace(source_filename, regexp_pattern, regexp_replace).

regexp_replace: See previous line. Can be empty. See docs of regexp_replace built in function.

Overwrite old file: If choose Y old file will be overwritten. If choose N old file [filename].[extension] will be renamed to [filename]_[to_char(systimestamp, 'YYYY-MM-DD-HH24-mi-ss-ff')].[extension].
...
Binary or text move: With text move, line breaks are replaced with line break defined in "Destination Linebreak" plugin attribute. If empty, then B (binary) is assumed.  

Charset: Oracle parameter of NLS_CHARSET_ID function. If empty UTF8 is used. Examples of valid values: UTF8, EE8MSWIN1250. It's only applicable when text move is used.

plsql block: PLSQL block to execute after file move. Can be empty.
